    ...
    useEffect(() => {
        ...
        const handleCellEdit = (rowIndex, cellIndex, newValue) => {
            ...
            if (columnName) {
                ...
                try {
                    ...
                    const response = await axios.put(`http://localhost:4000/api/serviceplan/${bookingId}`, updatedData, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                          },
                    });
        ...
        const handleDeleteRow = (rowIndex) => {
            ...
            const deleteData = async () => {
                try {
                    const response = await axios.delete(`http://localhost:4000/api/serviceplan/${tableData[rowIndex]._id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                          },
                    });
        ...
        const handleAddRow = (newRowData) => {
            const addData = async () => {
                try {
                    const response = await axios.post(`http://localhost:4000/api/serviceplan`, newRowData, {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                          },
                    });
